...
namespace Pcl4Editor
{
    ...
    public class BrushDetailNodeEditor : Editor
    {
        ...
        /// </summary>
        [MenuItem("GameObject/Pencil+ 4/Brush Detail Node", false)]
        public static void OpenBrushDetailNode()
        {
            List<GameObject> allGameObjs = Common.GetAllGameObject();
            GameObject newBrushDetail = Instantiate(Prefabs.BrushDetail);
            Undo.RegisterCreatedObjectUndo(newBrushDetail, "Create Brush Detail Node");
            newBrushDetail.name = allGameObjs.GetUniqueName(Prefabs.BrushDetail);
        }
    }
}
