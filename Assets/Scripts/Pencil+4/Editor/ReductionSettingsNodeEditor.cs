...
namespace Pcl4Editor
{
    ...
    public class ReductionSettingsNodeEditor : Editor
    {
        ...
        {
            ReductionSettingsNode prs = target as ReductionSettingsNode;
            serializedObject.Update();

            MakeReductionSettings(prs);
        ...
        /// </summary>
        [MenuItem("GameObject/Pencil+ 4/Reduction Settings Node", false)]
        public static void OpenReductionSettingsNode()
        {
            List<GameObject> allGameObjs = Common.GetAllGameObject();
            GameObject newReductionSettings = Instantiate(Prefabs.ReductionSettings);
            Undo.RegisterCreatedObjectUndo(newReductionSettings, "Create Reduction Settings Node");
            newReductionSettings.name = allGameObjs.GetUniqueName(Prefabs.ReductionSettings);
        }
    }
